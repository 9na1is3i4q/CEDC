...
public class DSA_CEDC extends DynamicSyncAgent {
    private final static int MSG_INITIAL_BROADCAST = 100001;
    private final static int MSG_ASK = 100002;
    ...
    private final static int MSG_NEW = 100006;
    private final static int MSG_CHECK = 100007;
    private final static int MSG_CHANGE = 100008;
    ...
    private final static double P = 0.4;

    public DSA_CEDC(DynamicEDCAgentDTO agentDTO, DynamicSyncMailer mailer, double gama, double costCE, double costEE) {
    ...
    public void disposeMessage(Message message) {
        switch (message.getType()){
            ...
            case MSG_VALUE:{
                ...
                checkAndSetResponseMessage();
                break;
            }
            case MSG_RESPONSE:{
                ResponseMessage responseMessage = (ResponseMessage) message.getValue();
                int sendId = responseMessage.getSendId();
                localAcceptRes.put(sendId,responseMessage);
                checkResponse();
                break;
            }
            case MSG_NEW:{
...
//            }
            ...
            if (check && broadcastTime<count){
                Print("满足要求，开始求最大值"+localAcceptTag);
                int[] newDataDelay = dataDelayView.getLocalDataDelay(transFormValueIndexToData(valueIndex));
...
//            }
    ...
    public void broadcastAskMsg(int minvalueIndex, double gain, int[] newDataDelay) {
        for (int i : allneighbours) {
            AskMessage askMessage = new AskMessage(id,minvalueIndex,gain,newDataDelay);
            Print("AskMessage: Send"+id +"->"+"Receive"+i+" Gain: "+gain+" newDataDelay: "+Arrays.toString(newDataDelay));
            sendMessage(new Message(id, i, MSG_ASK, askMessage));
        }
    }
    ...
    private void checkAndSetResponseMessage() {
        if(localAcceptValue.keySet().size() == allneighbours.length){
            calculateResponseMessage();
        }
    }

    private void calculateResponseMessage() {
        DataDelayView tempDataDelay = this.dataDelayView.deepCopy();
        bestValueIndex = selectBestValue();
        boolean[] check = new boolean[data.length];
        Arrays.fill(check,false);

        int[] tmpData1 = transFormValueIndexToData(bestValueIndex);
        for (int dataIndex=0;dataIndex<tmpData1.length;dataIndex++){
            if(tmpData1[dataIndex] == 1){
                check[dataIndex] = true;
            }
        }
        for(Integer agentID: localAcceptValue.keySet()){
            ValueMessage valueMessage = localAcceptValue.get(agentID);
            int valueIndex = valueMessage.getValueIndex();
        ...
        if(checkIsTrue(check)&&Math.random() < P){
            Print("Response True");
            broadcastResponseMsg(true);
            isAbleTochangeValue = true;
        }
        else {
//            if(maxAgentID == id){
//                Print("Response True");
//                broadcastResponseMsg(true,maxAgentID,maxGain);
//                isAbleTochangeValue = true;
//            }
//            else {
//                Print("Response False");
//                broadcastResponseMsg(false,maxAgentID,maxGain);
//                isAbleTochangeValue = false;
//            }
            Print("Response False");
            broadcastResponseMsg(false);
            isAbleTochangeValue = false;
        }
    ...
    private void broadcastResponseMsg(boolean check) {
        ResponseMessage responseMessage;
        for (int i : allneighbours) {
            responseMessage = new ResponseMessage(id,check);
            Print("ResponseMessage: Send"+id +"->"+"Receive"+i+" Check:"+check );
