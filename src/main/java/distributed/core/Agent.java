...
public abstract class Agent extends Process{
    ...
    protected Map<Integer, int[]> userRequireData;
    ...
    public Agent(EDCAgentDTO agentDTO) {
    ...
    public double calculateConstraintCacheBenfit(int neighbourId,int agentValueIndex,int neighbourValueIndex){
        ...
        double localCostPerData = 1.0 ;
        double neighborCostPerData = 1.0 ;
//        double localCostPerData = 1.0 / neighbours.length;
//        double neighborCostPerData = 1.0 / localView.get(neighbourId).adjectnum;
        double totalCost = (localCostPerData + neighborCostPerData)* data.length;
        double cost=0.0;
        for(int i=0;i<len;i++){
            cost += agentData[i]*localCostPerData+neighbourData[i]*neighborCostPerData;
        }
        return totalCost-cost;  //效益
    };
...
//        for (int neighbourId : neighbours){
//            localView.put(neighbourId,new NeighbourView());
//        }
        initRun();
        postInit();
    }
